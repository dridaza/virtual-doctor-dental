...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return NextResponse.json({ consentimientos: intake.consentimientos });
  } catch (err: any) {
    return NextResponse.json({ error: err?.message || 'No se pudieron cargar los consentimientos' }, { status: 502 });
  }
}

// Sube un documento de consentimiento (PDF, imagen, etc.) para descargar o
// imprimir después - a diferencia de la galería clínica, aquí se aceptan
// cualquier tipo de archivo, no solo imágenes.
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  const { id: contactId } = await params;
  try {
    const form = await request.formData();
    const file = form.get('file') as File | null;
    const name = String(form.get('name') || file?.name || 'consentimiento');
    if (!file) {
    ...
    const next = {
      ...migrated,
      consentimientos: [...migrated.consentimientos, { url, name, uploadedAt: new Date().toISOString() }],
    };
    await saveIntake(contactId, next);

    return NextResponse.json({ consentimientos: next.consentimientos });
  } catch (err: any) {
    return NextResponse.json({ error: err?.message || 'No se pudo subir el documento' }, { status: 502 });
  }
}
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const intake = await getIntake(contactId);
    const next = { ...intake, consentimientos: intake.consentimientos.filter((c) => c.url !== url) };
    await saveIntake(contactId, next);
    return NextResponse.json({ consentimientos: next.consentimientos });
  } catch (err: any) {
    return NextResponse.json({ error: err?.message || 'No se pudo eliminar el documento' }, { status: 502 });
  }
}
